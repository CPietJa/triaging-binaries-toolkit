...
static Data data[] = {
    {ELFDATANONE, "invalid", NULL, NULL, NULL, NULL},
    {ELFDATA2LSB, "little-endian", le8get, le16get, le32get, le64get},
    {ELFDATA2MSB, "big-endian", be8get, be16get, be32get, be64get}};
 ...
 * Read ELF32 Header
 */
static int readelf32ehdr(FILE *f, Fhdr *fp)
{
    uint8_t buf[Eh32sz];
    Elf32_Ehdr e;
    uint8_t *p;
    ...
    if (e.type != ET_REL && e.type != ET_EXEC && e.type != ET_DYN &&
        e.type != ET_CORE) {
 ...
 * Read ELF64 Header
 */
static int readelf64ehdr(FILE *f, Fhdr *fp)
{
    uint8_t buf[Eh64sz];
    Elf64_Ehdr e;
    uint8_t *p;
 ...
 */
int unpackelf32shdr(uint8_t *buf, int len, Elf32_Shdr *sh, Fhdr *fp)
{
    uint8_t *p;
 ...
 */
static int readelf32shdr(FILE *f, Fhdr *fp)
{
    uint8_t buf[Sh32sz];
    Elf32_Shdr sh;
 ...
 */
int unpackelf32phdr(uint8_t *buf, int len, Elf32_Phdr *ph, Fhdr *fp)
{
    uint8_t *p;
 ...
 */
static int readelf32phdr(FILE *f, Fhdr *fp)
{
    uint8_t buf[Ph32sz];
    Elf32_Phdr ph;
 ...
 */
int unpackelf64shdr(uint8_t *buf, int len, Elf64_Shdr *sh, Fhdr *fp)
{
    uint8_t *p;
 ...
 */
static int readelf64shdr(FILE *f, Fhdr *fp)
{
    uint8_t buf[Sh64sz];
    Elf64_Shdr sh;
 ...
 */
int unpackelf64phdr(uint8_t *buf, int len, Elf64_Phdr *ph, Fhdr *fp)
{
    uint8_t *p;
 ...
 */
static int readelf64phdr(FILE *f, Fhdr *fp)
{
    uint8_t buf[Ph64sz];
    Elf64_Phdr ph;
 ...
 */
int readelfphdrs(FILE *f, Fhdr *fp)
{
    unsigned int i;
 ...
 */
static int readelf32strndx(FILE *f, Fhdr *fp)
{
    uint8_t buf[Sh32sz];
    Elf32_Shdr sh;

    if (fseek(f, fp->shoff + (uint64_t)(fp->shstrndx * fp->shentsize),
              SEEK_SET) < 0)
        return -1;
 ...
 */
static int readelf64strndx(FILE *f, Fhdr *fp)
{
    uint8_t buf[Sh64sz];
    Elf64_Shdr sh;
...
static uint8_t *newsection(FILE *f, uint64_t offset, uint64_t size)
{
    uint8_t *sect;
 ...
 */
static int readelfstrndx(FILE *f, Fhdr *fp)
{
